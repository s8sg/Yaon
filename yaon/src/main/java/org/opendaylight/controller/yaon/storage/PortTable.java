...
public class PortTable extends HashTable {
	...
	private PortTable(DBConnection conn, HashMap<String, ArrayList<Object>> table){

		this.table = table;
		this.connection = conn;
	}

	public static PortTable init(DBConnection connection, String tableName){
		...
		PortTable.tableName = tableName;
		
		/* create table */
		HashMap<String, ArrayList<Object>> hashTable = HashTable.createTable(PortTable.tableName, fieldsNames);
		...
		PortTable table = new PortTable(connection, hashTable);
	...
	public boolean update(ArrayList<Object> primaryKeyValues,
		...
		for(Object fieldname : fieldsName){
			ArrayList<Object> storedFeildsName = this.table.get(PortTable.tableName);
